...
المطلوب منك تحليل شامل ومفصل يتضمن 3 أقسام رئيسية:
...
function generateBasicAnalysis(event: EconomicEvent): {
  ...
  const marketExpectation = event.forecast
    ? `السوق يتوقع ${event.forecast}. أي انحراف كبير عن هذا الرقم قد يسبب تحركات سعرية. المحللون يراقبون هذا الحدث بعناية لتقييم صحة الاقتصاد واتجاهاته المستقبلية.`
    : 'لا توجد توقعات محددة للسوق. ينتظر المتداولون صدور النتيجة لتقييم تأثيرها على الأسواق.';

  return {
    mainAnalysis,
    impact,
    marketExpectation,
    tradingRecommendation: '' // لم يعد مستخدماً
  };
}
...
function parseAIResponse(aiResponse: string): {
  ...
  if (!mainAnalysis || !impact || !marketExpectation) {
  ...
  console.log(`✅ Parsed sections: Analysis(${mainAnalysis.length}), Impact(${impact.length}), Expectation(${marketExpectation.length})`);
